...
#ifndef PBSL_META_TYPE_TRAITS_H_
#define PBSL_META_TYPE_TRAITS_H_
...
// feature.
    ...
    #define PBSL_INTERNAL_DEFAULT_NEW_ALIGNMENT __STDCPP_DEFAULT_NEW_ALIGNMENT__
#else  // defined(__STDCPP_DEFAULT_NEW_ALIGNMENT__)
    #define PBSL_INTERNAL_DEFAULT_NEW_ALIGNMENT alignof(std::max_align_t)
...
struct is_detected_impl            // NOLINT
{                                  // NOLINT
    using type = std::false_type;  // NOLINT
};

template < template < class... > class Op, class... Args >
struct is_detected_impl< typename VoidTImpl< Op< Args... > >::type,
                         Op,
                         Args... >  // NOLINT
{
    using type = std::true_type;  // NOLINT
};

template < template < class... > class Op, class... Args >
struct is_detected : is_detected_impl< void, Op, Args... >::type  // NOLINT
{
};  // NOLINT

template < class Enabler, class To, template < class... > class Op, class... Args >
struct is_detected_convertible_impl  // NOLINT
{                                    // NOLINT
    using type = std::false_type;    // NOLINT
};
...
template < class To, template < class... > class Op, class... Args >
struct is_detected_convertible                                     // NOLINT
    : is_detected_convertible_impl< void, To, Op, Args... >::type  // NOLINT
{
};
...
//
// NOTE: `phsl::void_t` does not use the standard-specified implementation so
// that it can remain compatible with gcc < 5.1. This can introduce slightly
// different behavior, such as when ordering partial specializations.
template < typename... Ts >
using void_t = typename type_traits_internal::VoidTImpl< Ts... >::type;  // NOLINT
...
template < typename... Ts >
struct conjunction : std::true_type  // NOLINT
{
};

template < typename T, typename... Ts >
struct conjunction< T, Ts... > : std::conditional< T::value, conjunction< Ts... >, T >::type  // NOLINT
{
};
...
template < typename... Ts >
struct disjunction : std::false_type  // NOLINT
{
};

template < typename T, typename... Ts >
struct disjunction< T, Ts... > : std::conditional< T::value, T, disjunction< Ts... > >::type  // NOLINT
{
};
...
template < typename T >
struct negation : std::integral_constant< bool, !T::value >  // NOLINT
{
};
...
template < typename T >
using remove_cvref = std::remove_cvref< T >;  // NOLINT

template < typename T >
using remove_cvref_t = typename std::remove_cvref< T >::type;  // NOLINT
#else
// remove_cvref()
//
...
// C++20.
template < typename T >
struct remove_cvref  // NOLINT
{
    using type = typename std::remove_cv< typename std::remove_reference< T >::type >::type;  // NOLINT
};

template < typename T >
using remove_cvref_t = typename remove_cvref< T >::type;  // NOLINT
#endif
...
template < typename T >
using remove_cv_t = typename std::remove_cv< T >::type;  // NOLINT

template < typename T >
using remove_const_t = typename std::remove_const< T >::type;  // NOLINT

template < typename T >
using remove_volatile_t = typename std::remove_volatile< T >::type;  // NOLINT

template < typename T >
using add_cv_t = typename std::add_cv< T >::type;  // NOLINT

template < typename T >
using add_const_t = typename std::add_const< T >::type;  // NOLINT

template < typename T >
using add_volatile_t = typename std::add_volatile< T >::type;  // NOLINT

template < typename T >
using remove_reference_t = typename std::remove_reference< T >::type;  // NOLINT

template < typename T >
using add_lvalue_reference_t = typename std::add_lvalue_reference< T >::type;  // NOLINT

template < typename T >
using add_rvalue_reference_t = typename std::add_rvalue_reference< T >::type;  // NOLINT

template < typename T >
using remove_pointer_t = typename std::remove_pointer< T >::type;  // NOLINT

template < typename T >
using add_pointer_t = typename std::add_pointer< T >::type;  // NOLINT

template < typename T >
using make_signed_t = typename std::make_signed< T >::type;  // NOLINT

template < typename T >
using make_unsigned_t = typename std::make_unsigned< T >::type;  // NOLINT

template < typename T >
using remove_extent_t = typename std::remove_extent< T >::type;  // NOLINT

template < typename T >
using remove_all_extents_t = typename std::remove_all_extents< T >::type;  // NOLINT

template < typename T >
using decay_t = typename std::decay< T >::type;  // NOLINT

template < bool B, typename T = void >                      // NOLINT
using enable_if_t = typename std::enable_if< B, T >::type;  // NOLINT

template < bool B, typename T, typename F >                        // NOLINT
using conditional_t = typename std::conditional< B, T, F >::type;  // NOLINT

template < typename... T >
using common_type_t = typename std::common_type< T... >::type;  // NOLINT

template < typename T >
using underlying_type_t = typename std::underlying_type< T >::type;  // NOLINT
...
struct result_of< F(Args...) > : std::invoke_result< F, Args... >  // NOLINT
{
};
#else
template < typename F >
using result_of = std::result_of< F >;  // NOLINT
#endif

}  // namespace type_traits_internal

template < typename F >
using result_of_t = typename type_traits_internal::result_of< F >::type;  // NOLINT
...
//
    ...
    #define PBSL_META_INTERNAL_STD_HASH_SFINAE_FRIENDLY_ 0
#else
    #define PBSL_META_INTERNAL_STD_HASH_SFINAE_FRIENDLY_ 1
#endif

#if !PBSL_META_INTERNAL_STD_HASH_SFINAE_FRIENDLY_
template < typename Key, typename = size_t >
struct IsHashable : std::true_type
{
};
#else   // PBSL_META_INTERNAL_STD_HASH_SFINAE_FRIENDLY_
...
struct IsHashable< Key,
                   phsl::enable_if_t< std::is_convertible< decltype(std::declval< std::hash< Key > & >()(  // NOLINT
                                                               std::declval< Key const & >())),
                                                           std::size_t >::value > > : std::true_type
{
};
#endif  // !PBSL_META_INTERNAL_STD_HASH_SFINAE_FRIENDLY_
...
private:
    ...
    static std::nullptr_t DoIt()
    {
        ...
        static_assert(phsl::is_copy_assignable< std::hash< Key > >::value,
...
// This declaration prevents global `swap` and `phsl::swap` overloads from being
...
// arguments of type `T`.
template < class T >
struct IsSwappable : phsl::type_traits_internal::is_detected< IsSwappableImpl, T >  // NOLINT
{
};
...
struct IsNothrowSwappable
    : phsl::type_traits_internal::is_detected< IsNothrowSwappableImpl,
                                               T >  // NOLINT
{
};
...
template < class T, phsl::enable_if_t< IsSwappable< T >::value, int > = 0 >  // NOLINT  // NOLINT
...
// phsl::is_trivially_relocatable<T>
//
...
// implementation, but phsl::is_trivially_relocatable<std::vector<int>> remains
// false.
//
// Example:
//
// if constexpr (phsl::is_trivially_relocatable<T>::value) {
...
//
// TODO:(b/325479096): Remove the opt-out once Clang's behavior is
// fixed.
//
// According to https://github.com/abseil/abseil-cpp/issues/1479, this does not
// work with NVCC either.
#if PBSL_HAVE_BUILTIN(__is_trivially_relocatable)                                                                      \
...
// relocatable.
template < class T >
struct is_trivially_relocatable : std::is_trivially_copyable< T >  // NOLINT
{
};
#endif

// phsl::is_constant_evaluated()
//
...
//
// Applications can inspect `PBSL_HAVE_CONSTANT_EVALUATED` at compile time
...
//   if (!phsl::is_constant_evaluated()) {
//     PBSL_LOG(INFO) << "MyClass(" << param << ")";
//   }
// #endif  // PBSL_HAVE_CONSTANT_EVALUATED
// }
//
// Upstream documentation:
//
...
//
#if defined(PBSL_HAVE_CONSTANT_EVALUATED)
constexpr bool is_constant_evaluated() noexcept  // NOLINT
{
...
// declaring
//   using pbsl_internal_is_view = std::true_type;
// as a member.
...
{
    static_assert(std::is_same< T, phsl::remove_cvref_t< T > >::value, "type must lack qualifiers");
};

template < typename T >
struct IsOwnerImpl< T, std::enable_if_t< std::is_class< typename T::pbsl_internal_is_view >::value > >
    : phsl::negation< typename T::pbsl_internal_is_view >
{
};
...
template < typename T >
struct IsViewImpl< T, std::enable_if_t< std::is_class< typename T::pbsl_internal_is_view >::value > >
    : T::pbsl_internal_is_view
{
};
...
using IsLifetimeBoundAssignment = std::integral_constant< bool,
                                                          IsView< phsl::remove_cvref_t< T > >::value
                                                              && IsOwner< phsl::remove_cvref_t< U > >::value >;
...
#endif  // PBSL_META_TYPE_TRAITS_H_
